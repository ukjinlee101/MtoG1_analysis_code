...
#        here(loopDir, paste0("chromo_union_allRes_postprocessed_25kb.bedpe")), sep = "\t",
#        col.names = FALSE)
# 
# fwrite(loops.10kb %>% dplyr::select(seq(1, 6)), 
#        here(loopDir, paste0("chromo_union_allRes_postprocessed_10kb.bedpe")), sep = "\t",
#        col.names = FALSE)
# 
# fwrite(loops.5kb %>% dplyr::select(seq(1, 6)), 
#        here(loopDir, paste0("chromo_union_allRes_postprocessed_5kb.bedpe")), sep = "\t",
#        col.names = FALSE)
# 
# loops %>% dplyr::filter(size %in% c(1000, 2000, 5000))
```
...
### Annotate anchors
```{r}
loop <- as_tibble(fread(here(loopDir, "Hansen_union_allRes.bedpe")))
...
fwrite(anchor.tb, here(loopDir, "Hansen_union_allRes_anchor.bed"), sep = "\t", col.names = FALSE)
...
svglite(here(figDir, "anchorClassify_Hansen_union_allRes_chromHMMRAD21.svg"), width = 3, height = 5)
print(p)
dev.off()
```
### Annotate loops
```{r}
anchor.tb <- fread(here(loopDir, "Hansen_union_allRes_anchor.bed")) %>%
  dplyr::mutate(anchor_id = paste(V1, V2, V3, sep = "_")) %>%
  rename(annot = V5)

consensus.loop.anno.tb <- fread(here(loopDir, "Hansen_union_allRes.bedpe"))
