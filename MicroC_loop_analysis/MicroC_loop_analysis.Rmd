...
#### Importing ChIP-seq peaks and expanding them
```{r}
# IMPORTING PEAKS
chipDir <- here("..", "reference", "ChIP")

peak.H3K27ac.EpiLC <- makeGRangesFromDataFrame(fread(
  here(chipDir, "temp", "H3K27ac_diff_specific_EpiLC.bed"),
  select = 1:3,
  col.names = c("chr", "start", "end")
))
peak.H3K27ac.ESC <- makeGRangesFromDataFrame(fread(
  here(chipDir, "temp", "H3K27ac_diff_specific_ESC.bed"),
  select = 1:3,
  col.names = c("chr", "start", "end")
))
peak.H3K27ac.common <- makeGRangesFromDataFrame(fread(
  here(chipDir, "temp", "H3K27ac_diff_noChange.bed"),
  select = 1:3,
  col.names = c("chr", "start", "end")
))

peak.H3K4me3.EpiLC <- makeGRangesFromDataFrame(fread(
  here(chipDir, "temp", "H3K4me3_diff_specific_EpiLC.bed"),
  select = 1:3,
  col.names = c("chr", "start", "end")
))
peak.H3K4me3.ESC <- makeGRangesFromDataFrame(fread(
  here(chipDir, "temp", "H3K4me3_diff_specific_ESC.bed"),
  select = 1:3,
  col.names = c("chr", "start", "end")
))
peak.H3K4me3.common <- makeGRangesFromDataFrame(fread(
  here(chipDir, "temp", "H3K4me3_diff_noChange.bed"),
  select = 1:3,
  col.names = c("chr", "start", "end")
))

peak.Epi <- c(peak.H3K27ac.EpiLC, peak.H3K4me3.EpiLC)
peak.ESC <- c(peak.H3K27ac.ESC, peak.H3K4me3.ESC)
...
index <- unique(c(queryHits(findOverlaps(first(loop), peak.Epi)),
                     queryHits(findOverlaps(second(loop), peak.Epi))))
loop.Epi <- loop[index]
index <- unique(c(queryHits(findOverlaps(first(loop), peak.ESC)),
                     queryHits(findOverlaps(second(loop), peak.ESC))))
loop.ESC <- loop[index]

# fwrite(as_tibble(loop.Epi) %>% dplyr::select(c(1, 2, 3, 6, 7, 8)), here(dataDir, "temp_loop_epi.bedpe"), sep = "\t", col.names = FALSE)
# fwrite(as_tibble(loop.ESC) %>% dplyr::select(c(1, 2, 3, 6, 7, 8)), here(dataDir, "temp_loop_esc.bedpe"), sep = "\t", col.names = FALSE)

temp.Epi <- as_tibble(loop.Epi) %>% dplyr::mutate(id = paste(seqnames1, start1-1, end1, seqnames2, start2-1, end2, sep = "_"))
temp.ESC <- as_tibble(loop.ESC) %>% dplyr::mutate(id = paste(seqnames1, start1-1, end1, seqnames2, start2-1, end2, sep = "_"))
...
diffCutoff <- 0.2
  ...
  temp <- data_to_plot %>%
  dplyr::mutate(density = get_density(score_DMSO, score_dTAG, n = 200))
  density_limits <- range(temp$density, na.rm = TRUE)
diffCutoff <- 0.2

  # ------ Scatterplot ------
  p1 <- ggplot(temp, aes(x = score_DMSO, y = score_dTAG)) +
    # Layer 1: Plot "NO" points with a grey scale
    geom_point(data = . %>% arrange(density),
               aes(color = density), size = 0.1) +
    # Add annotations and theme elements
  ...
  name = "ChIP_diff_Epispecific_dTAGvsDMSO"
  fileName <- paste0("scatterplot_", name)
  ...
  dev.off()
   ...
   temp <- data_to_plot %>%
  dplyr::mutate(density = get_density(score_EpiDMSO, score_EpidTAG, n = 200))
  density_limits <- range(temp$density, na.rm = TRUE)
diffCutoff <- 0.2

  # ------ Scatterplot ------
  p1 <- ggplot(temp, aes(x = score_EpiDMSO, y = score_EpidTAG)) +
    # Layer 1: Plot "NO" points with a grey scale
    geom_point(data = . %>% arrange(density),
               aes(color = density), size = 0.1) +
    # Add annotations and theme elements
  ...
  name = "ChIP_diff_Epispecific_EpidTAGvsEpiDMSO"
  fileName <- paste0("scatterplot_", name)
...
pseudo_ooe = 0.01
  plot_df_ooe <- temp %>%
    ...
    dplyr::filter(!is.na(ooe_DMSO), !is.na(ooe_dTAG), !is.na(ooe_EpiDMSO), !is.na(ooe_EpidTAG)) %>%
    dplyr::mutate(
      ooe_DMSO = if_else(ooe_DMSO == 0, pseudo_ooe, ooe_DMSO),
      ooe_dTAG = if_else(ooe_dTAG == 0, pseudo_ooe, ooe_dTAG),
      ooe_EpiDMSO = if_else(ooe_EpiDMSO == 0, pseudo_ooe, ooe_EpiDMSO),
  ...
  plot_df_ooe <- plot_df_ooe %>% dplyr::select(fc_ESC, fc_Epi) %>% pivot_longer(everything(), names_to = "condition")

  plot_df_ooe$condition <- factor(plot_df_ooe$condition, levels = c("fc_ESC", "fc_Epi"))
  
  p3 <- ggplot(plot_df_ooe, aes(x = condition, y = value, fill = condition)) +
    geom_violin(linewidth = lineThick * mmToLineUnit, lineend = "square", alpha = .4, show.legend = FALSE) + 
    geom_boxplot(width = 0.3, color = "black",
                 linewidth = lineThick * mmToLineUnit, lineend = "square",
                 outlier.shape = NA,  alpha = 0.6, show.legend = FALSE) + 
...
# Annotation of those loops




#
loop.reg <- data %>% dplyr::filter(Anno %in% c("P-P", "P-E", "E-E")) %>%
  dplyr::select(seq(1, 6))
colnames(loop.reg) <- c("V1", "V2", "V3", "V4", "V5", "V6")
loop.reg <- importBedpe(loop.reg)
loop.str <- data %>% dplyr::filter(Anno %in% c("S-S", "S-X")) %>%
  dplyr::select(seq(1, 6))
colnames(loop.str) <- c("V1", "V2", "V3", "V4", "V5", "V6")
loop.str <- importBedpe(loop.str)
#


index <- unique(c(queryHits(findOverlaps(first(loop.reg), peak.Epi)),
                     queryHits(findOverlaps(second(loop.reg), peak.Epi))))
loop.reg.Epi <- loop.reg[index]
index <- unique(c(queryHits(findOverlaps(first(loop.reg), peak.ESC)),
                     queryHits(findOverlaps(second(loop.reg), peak.ESC))))
loop.reg.ESC <- loop.reg[index]
index <- unique(c(queryHits(findOverlaps(first(loop.str), peak.Epi)),
                     queryHits(findOverlaps(second(loop.str), peak.Epi))))
loop.str.Epi <- loop.str[index]
index <- unique(c(queryHits(findOverlaps(first(loop.str), peak.ESC)),
                     queryHits(findOverlaps(second(loop.str), peak.ESC))))
loop.str.ESC <- loop.str[index]


as_tibble(loop.reg.Epi) %>% dplyr::select(seq(1, 6))




fwrite(as_tibble(loop.reg.Epi) %>% dplyr::select(c(1, 2, 3, 6, 7, 8)), here(dataDir, "temp_loop_reg_epi.bedpe"), sep = "\t", col.names = FALSE)
fwrite(as_tibble(loop.reg.ESC) %>% dplyr::select(c(1, 2, 3, 6, 7, 8)), here(dataDir, "temp_loop_reg_esc.bedpe"), sep = "\t", col.names = FALSE)

fwrite(as_tibble(loop.str.Epi) %>% dplyr::select(c(1, 2, 3, 6, 7, 8)), here(dataDir, "temp_loop_str_epi.bedpe"), sep = "\t", col.names = FALSE)
fwrite(as_tibble(loop.str.ESC) %>% dplyr::select(c(1, 2, 3, 6, 7, 8)), here(dataDir, "temp_loop_str_esc.bedpe"), sep = "\t", col.names = FALSE)
