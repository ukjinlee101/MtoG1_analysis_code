...
geneAnnoData <- loadLoopAnnoData(here(commonLoopDir, paste0("loopScore_", name, "_p-n_complex_ensemblList.tsv"))) %>%
  ...
  dplyr::mutate(group = case_when(
    gene %in% geneList.group1$gene ~ "group1",
    gene %in% geneList.group2$gene ~ "group2",
    TRUE ~ NA
  )) %>%
  dplyr::filter(!is.na(group))
...
fileName <- paste0("Loop_geneDensity")
width <- panelSize(1.5)*mmToInch
height <- panelSize(1.2)*mmToInch
svglite(here(figDir, paste0(fileName, ".svg")),  height = height, width = width)
print(p)
...
my_gene_lists <- list(
  "ESC Down" = geneList.escdown.RNA,
  "Epi Down" = geneList.epidown.RNA,
  "Epi Long Down" = geneList.epiLongdown.RNA
)
...
p <- plot(
  fit,
  fills = list(fill = c(strong_orange, "#009B77", darken("#009B77", amount = 0.5)), alpha = 0.7),
  edges = list(lwd = 0.75),
  labels = list(fontfamily = "Helvetica", fontsize = 6),
  quantities = list(type = c("counts"), fontfamily = "Helvetica", fontsize = 6),
)
name <- paste0("venn_DOWNDEG_comparison_")
width <- panelSize(1)*mmToInch
height <- panelSize(1)*mmToInch  
svglite(here(figDir,
             paste0(name, ".svg")),
...
common <- sum(geneList.escdown.RNA %in% geneList.epidown.RNA)

fit <- euler(c(
  "ESC" = length(geneList.escdown.RNA) - common ,
  "EpiLC" = length(geneList.epidown.RNA) - common,
  "ESC&EpiLC"       = common
))
...
fileName <- here(figDir, "Venn_DEGs_DOWN")
width <- panelSize(1)*mmToInch
height <- panelSize(1)*mmToInch
svglite(paste0(fileName, ".svg"), width = width, height =height)
print(p)
