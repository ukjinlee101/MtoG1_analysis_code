...
### Checking how many of the down DEGs has any perturbed loops
data_to_plot <- fread(here(commonLoopDir, "loopScore_chromosight_extra.tsv"))
...
generate_scatterplot(data, "score_DMSO",    "score_UT", "allLoops_AsyncUTvsG1DMSO")
generate_scatterplot(data, "score_UT",    "score_AID", "allLoops_AsyncAIDvsAsyncUT")
generate_scatterplot(data, "score_DMSO",    "score_dTAG", "allLoops_G1dTAGvsG1DMSO")
...
generate_scatterplot2(
  ...
  x_var = "delta_G1",
  y_var = "delta_Async",
  name = "allLoops_deltaScore_AsyncvsG1",
  limits = c(-0.75, 0.75), # Use default data limits
  diff_cutoff = 0.2
)
...
fwrite((temp %>% dplyr::select(seq(1, 6))), here(dataDir, "loop_deltaScore_G1specificPerturb.bedpe"), sep = "\t", col.names = FALSE)

generate_scatterplot2(
  df = temp,
  x_var = "delta_G1",
  y_var = "delta_Async",
  name = "allLoops_deltaScore_AsyncvsG1_G1specific",
  limits = c(-0.75, 0.75), # Use default data limits
  diff_cutoff = 0.2
)
...
fwrite((temp %>% dplyr::select(seq(1, 6))), here(dataDir, "loop_deltaScore_AsyncspecificPerturb.bedpe"), sep = "\t", col.names = FALSE)

generate_scatterplot2(
  df = temp,
  x_var = "delta_G1",
  y_var = "delta_Async",
  name = "allLoops_deltaScore_AsyncvsG1_Asyncspecific",
  limits = c(-0.75, 0.75), # Use default data limits
  diff_cutoff = 0.2
)
...
loop <- data %>% dplyr::filter(id %in% loop.G1SpecificPerturb$id)
temp1 <- make_table(loop, "G1SpecificPerturb")
loop <- data %>% dplyr::filter(id %in% loop.AsyncSpecificPerturb$id)
temp2 <- make_table(loop, "AsyncSpecificPerturb")
