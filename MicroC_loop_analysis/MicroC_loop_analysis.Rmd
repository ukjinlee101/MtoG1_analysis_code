...
```{r}
getOverlapLoopNumExpand <- function(loop, peak){
  anchor <- resize(extractAnchor(loop), width = 25000, fix = "center")
  a <- unique(queryHits(findOverlaps(anchor, peak)))
  return(length(a))
}
...
getSEOverlapFisher <- function(allLoop, subsetLoop, peak){
  all.overlap <- getOverlapLoopNumExpand(allLoop, peak)
  all.notOverlap <- nrow(allLoop) - all.overlap
  
  subset.overlap <- getOverlapLoopNumExpand(subsetLoop, peak)
