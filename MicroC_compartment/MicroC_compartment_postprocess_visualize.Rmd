...
expName = "EpiG1DMSO_pooled"
contacts.EpiG1DMSO <- readRDS(here(rdsDir, paste0(expName, "_100000bp_KRbalanced.rds")))
expName = "EpiG1dTAG_pooled"
contacts.EpiG1dTAG <- readRDS(here(rdsDir, paste0(expName, "_100000bp_KRbalanced.rds")))
```
...
drawSaddlePlot <- function(contact1, contact2, cond1, cond2, expName, binSize = 100000, metaBin = 50, compScorePerc = 0.1){
  ...
  for(j in c(1:nrow(idxChr))){
    ...
    metaAB <- ggplot(saddle.plot$data %>% dplyr::filter(panel == "Individual"),
                     aes(x = q1, y = q2)) +
      geom_tile(aes(fill = pmax(pmin(log10(obsexp), oelim), -oelim))) +
      facet_grid(~exp) +
      coord_fixed() +
      scico::scale_fill_scico(palette = "vik", limits = c(-oelim, oelim)) +
      labs(fill = "log10(O/E)") + xlab(NULL) + ylab(NULL) + theme_classic()
    ...
    metaAB_Diff = ggplot(diffSaddle[[2]], aes(x=q1,y=q2)) +
      geom_tile(aes(fill=
                      pmax(pmin(log10(diffSaddle[[2]]$obsexp/diffSaddle[[1]]$obsexp), diffoelim), -diffoelim))) +
      coord_fixed() +  facet_grid(~exp) +
      scico::scale_fill_scico(palette = "cork", limits = c(-diffoelim, diffoelim)) +
      labs(fill='diff_log10(O/E)') +  xlab(NULL) + ylab(NULL)+theme_classic()
...
drawSaddlePlot(contacts.G1DMSO, contacts.G1A485, "G1DMSO_pooled", "G1A485_pooled",
               "G1DMSO_G1A485")
drawSaddlePlot(contacts.EpiG1DMSO, contacts.EpiG1dTAG, "EpiG1DMSO_pooled", "EpiG1dTAG_pooled",
               "EpiG1DMSO_EpiG1dTAG")
drawSaddlePlot(contacts.G1DMSO, contacts.EpiG1DMSO, "G1DMSO_pooled", "EpiG1DMSO_pooled",
               "G1DMSO_EpiG1DMSO")
```
...
getPvalWilcox <- function(data, cond1, cond2){
  distance1 <- (data %>% dplyr::filter(exp ==cond1) )$compScore
  distance2 <- (data %>% dplyr::filter(exp ==cond2) )$compScore
...
drawCompScore <- function(cond1, cond2, expName){
  ...
  pval <- getPvalWilcox(data, cond1, cond2)
  
  plot <- ggplot(data, aes(x = exp, y = compScore)) +
    geom_boxplot(width = 0.6, outlier.shape = NA, color = "black", fill = c(strong_grey, strong_orange),
                 linewidth = lineMedium * mmToLineUnit) +
    geom_beeswarm(color = "black", shape = 16, size = 0.5) +
    ggtitle("Compartment Score") +
    ylab("Compartment Strength\n(AA+BB)/(AB+BA)") +
    ylim(2, 8) +theme_classic() +
    theme(
    ...
    ) +  annotate("text", x = 1, y = 5, label = paste0("", convPvalue(pval)), 
                  color = "black", hjust = 0, size = 2)
  
  fileName = here(outDir, paste0("compScore_summary"))
  width <- panelSize(0.85)*mmToInch
...
drawCompScore("EpiG1DMSO_pooled", "EpiG1dTAG_pooled",
               "EpiG1DMSO_EpiG1dTAG")
drawCompScore("G1DMSO_pooled", "EpiG1DMSO_pooled",
               "G1DMSO_EpiG1DMSO")
```
...
drawAABBDiff <- function(cond1, cond2, expName, ymin = -0.25, ymax = 0.5){
  ...
  p <- ggplot(diffScore.long, aes(x = compartment, y = diffScore)) +
        geom_boxplot(width = 0.6, outlier.shape = NA, color = "black", fill = strong_grey,
                 linewidth = lineMedium * mmToLineUnit) +
    geom_beeswarm(color = "black", shape = 16, size = 0.5) +
    geom_hline(yintercept = 0, linewidth = lineThick * mmToLineUnit, lineend = "square") +
    coord_cartesian(ylim = c(ymin, ymax)) +theme_classic() +
    theme(
      legend.position = "none",
      plot.title = element_text(
        hjust = 0.5,
...
drawAABBDiff("EpiG1DMSO_pooled", "EpiG1dTAG_pooled",
               "EpiG1DMSO_EpiG1dTAG")
drawAABBDiff("G1DMSO_pooled", "EpiG1DMSO_pooled",
               "G1DMSO_EpiG1DMSO")
```
