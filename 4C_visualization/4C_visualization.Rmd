...
makeNorm4CBigwig <- function(VP, name, expName, bwDir, binSize, shiftSize){
  
  reads.1 <- readRDS(here(rdsDir, paste0("4C_", VP, "_", name, "_", expName, ".rds")))$reads
  ...
  seqlevels(bin.Gr) <- seqlevels(seqinfo_object)
  seqinfo(bin.Gr) <- seqinfo_object
  export(bin.Gr, here(bwDir, paste0(VP, "_", name, "_", expName, ".bw")), format = "BigWig")
...
VP <- "Klf4"
name <- "ESC-G1DMSO"
makeNorm4CBigwig(VP, name, "Exp1", bwDir, binSize, shiftSize)
makeNorm4CBigwig(VP, name, "Exp2", bwDir, binSize, shiftSize)
makeNorm4CBigwig(VP, name, "Exp3", bwDir, binSize, shiftSize)
name <- "ESC-G1dTAG"
makeNorm4CBigwig(VP, name, "Exp1", bwDir, binSize, shiftSize)
makeNorm4CBigwig(VP, name, "Exp3", bwDir, binSize, shiftSize)
name <- "EpiLC-G1DMSO"
...
data <- data.frame(
  Timepoint = c("0 h", "3 h", "8 h", "22 h", "8 h", "22 h"),
  HA = c(3.087898775, 6.715696649, 52.62699565, 80.22056539, 51.65897991, 102.5480094)
)
...
p <- ggplot(data_summary, aes(x = Timepoint, y = Mean_HA)) +
  # Bar plot for means
  geom_bar(stat = "identity", fill = "grey50", color = "black", width = 0.75) +
  # Error bars for SE
