...
# Create the data frame
data <- data.frame(
  Timepoint = c("22 h DM", "22 h DM",
                "22 h dT", "22 h dT",
                "48 h DM", "48 h DM", "48 h DM", 
                "48 h dT", "48 h dT", "48 h dT"),
  HA = c(73.09441697, 71.68242966,
         63.18523465, 63.10380639,
         133.6224654, 118.0649558, 150.487477,
         94.38613912, 85.58628617, 116.7908954)
)

# Convert Timepoint to an ordered factor (ensures correct plotting order)
data$Timepoint <- factor(data$Timepoint, levels = c("22 h DM", "22 h dT", "48 h DM", "48 h dT"))
...
p <- ggplot(data_summary, aes(x = Timepoint, y = Mean_HA)) +
  ...
  # Error bars for SE
  geom_jitter(data = data, aes(x = Timepoint, y = HA),
              color = "black", size = 2/10, width = 0.1, shape = 16) +
  geom_errorbar(aes(ymin = Mean_HA - SE, ymax = Mean_HA + SE), 
                width = 0.2, color = "black", size = lineThick * mmToLineUnit) +
  # Labels and Theme
  labs(
...
## [FIG] Release after dTAG, Stacked barplot
```{r}
# Data
data <- data.frame(
...
## [FIG] ChIP Venn diagram
### RAD21 DMSO vs dTAG
```{r}
chipDir <- here("../reference", "ChIP", "RAD21-HA")
bed1 <- import(here(chipDir, "UnionPeaks_RAD21_ESC-DMSO_and_dTAG__ESC-DMSO.bed"), format="BED")
bed2 <- import(here(chipDir, "UnionPeaks_RAD21_ESC-DMSO_and_dTAG__ESC-dTAG.bed"), format="BED")

# compute overlaps and uniques
ov1 <- overlapsAny(bed1, bed2)
ov2 <- overlapsAny(bed2, bed1)

# partition into three sets
common <- bed1[ov1]             # all bed1 ranges that overlap bed2
only1  <- bed1[!ov1]            # bed1 ranges with no overlap
only2  <- bed2[!ov2]            # bed2 ranges with no overlap
...
p <- plot(
  ...
  labels = list(
    font = 1,         # font face: 1=plain, 2=bold, 3=italic, 4=bold italic
    cex = 1,        # label font size multiplier
    family = fontType # font family name
  ),
  quantities = list(
    cex = 1,        # quantity font size multiplier
    font = 1,         # font face for numbers
    family = fontType
  ),
  edges = list(
    lwd = 2           # line width in points
  ),
  main = "ChIP-seq | RAD21 ESC-dTAG peaks",
  main.pos = c(0.5, 1.05), # adjust title position
  main.gp = gpar(
    fontsize = fontSizeL,    # title font size in points
    fontfamily = fontType,
    fontface = "bold"
  )
)
fileName <- here(figDir, "Venn_ChIP_RAD21_dTAG")
width <- panelSize(1)*mmToInch
height <- panelSize(1)*mmToInch
svglite(paste0(fileName, ".svg"), width = width, height =height)
print(p)
...
```
### RAD21/HA ESC vs EpiLC
```{r}
chipDir <- here("../reference", "ChIP", "RAD21-HA")
bed1 <- import(here(chipDir, "UnionPeaks_RAD21-HA_ESC-DMSO_and_EpiLC__ESC-DMSO.bed"), format="BED")
bed2 <- import(here(chipDir, "UnionPeaks_RAD21-HA_ESC-DMSO_and_EpiLC__EpiLC.bed"), format="BED")
...
fileName <- here(figDir, "Venn_ChIP_RAD21-HA_ESC_vs_EpiLC")
width <- panelSize(1)*mmToInch
height <- panelSize(1)*mmToInch
svglite(paste0(fileName, ".svg"), width = width, height =height)
print(p)
