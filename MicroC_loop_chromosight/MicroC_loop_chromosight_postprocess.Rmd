...
pkgs = c("tidyverse", "here", "svglite", "ggplot2", "ggrepel",
         ...
         "InteractionSet", "gplots", "locfdr", "future", "ggbeeswarm", "glossary", "ggalluvial")
...
for (res in c(25000, 10000, 5000)){
  ...
  score.tb <- full_join(score.DMSO.tb, score.dTAG.tb,
    ...
    dplyr::mutate(
      score = pmax(score_DMSO, score_dTAG, score_A485),
  ...
  dms_vec   <- score.tb$DMSOcalled
  dtag_vec  <- score.tb$dTAGcalled
  a485_vec  <- score.tb$A485called
  ...
  # render & save
  svglite(here(figDir, paste0("euler_DMSO_dTAG_A485_raw_", res, "bp.svg")), width = 4, height = 4)
  print(plot(
    fit,
    ...
    main       = paste0("Overlap: ", res, "bp, raw")
  ))
  dev.off()
  
  
  dms_vec   <- temp$DMSOcalledProcessed
  dtag_vec  <- temp$dTAGcalledProcessed
  a485_vec  <- temp$A485calledProcessed
  ...
  # render & save
  svglite(here(figDir, paste0("euler_DMSO_dTAG_A485_processed_", res, "bp.svg")), width = 4, height = 4)
  print(plot(
    fit,
    fills      = list(fill = c("#1f77b4", "#ff7f0e", "#2ca02c"), alpha = 0.5),
    edges      = FALSE,
    labels     = list(fontsize = 16),
    quantities = list(type = "counts", fontface = "bold", fontsize = 14),
    main       = paste0("Overlap: ", res, "bp, postprocessed")
  ))
  dev.off()
  ...
  fwrite(temp, here(loopDir, paste0("chromo_cons_", res, "bp_postprocessed.bedpe")), sep = "\t", col.names = FALSE)
  
  out.tb <- score.tb %>% dplyr::filter(is_best) %>% dplyr::select(-group, -is_best, -DMSOcalled, -dTAGcalled, -A485called)
  fwrite(out.tb, here(loopDir, paste0("chromo_cons_", res, "bp_postprocessed.tsv")), sep = "\t")
}


```

## Step 2 (EpiLC expanded)
```{r}
################################################################################
# Step 2
################################################################################
...
for (res in c(25000, 10000, 5000)){
  ...
  fwrite(loop.merged, here(loopDir, paste0("chromo_cons_", res, "bp_fullset.bedpe")), 
         sep = '\t', col.names = FALSE, row.names = FALSE)
}
...
peak.RAD21 <- importPeak(here(refDir, "ChIP","RAD21_ESC_unionPeaks.bed"))
...
fwrite(temp, here(loopDir, "chromo_cons_allRes_postprocessed_anchor.bed"), sep = "\t", col.names = FALSE)
